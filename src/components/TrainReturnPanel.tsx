...
import { fetchTrainReturn, type TrainInfo } from "@/lib/apiClient";
...
export default function TrainReturnPanel({
  ...
  const [info, setInfo] = useState<TrainInfo | null>(null);
  ...
  return (
    <div className="rounded-md border border-zinc-300 dark:border-zinc-700 p-3 text-sm">
      <h3 className="font-medium mb-2">Zugrückfahrt</h3>
      ...
      {info && info.configured && "trips" in info && (
        <div className="flex flex-col gap-2 text-xs">
          <p>
            {info.destStation.name} → {info.homeStation.name}
          </p>
